...
import { bytesEqual, concatBytes } from "./utils.ts";
...
export type TextEnvelopeDescription = Omit<TextEnvelopeInfo, "text">;

function ensureText(value: unknown): asserts value is string {
...
function readUint32BigEndian(input: Uint8Array, offset: number): number {
  return (
...
function getPaddingLength(payloadLength: number): number {
  return payloadLength % 2;
}

function hasMagic(bytes: unknown): bytes is Uint8Array {
  if (Object.prototype.toString.call(bytes) !== "[object Uint8Array]") {
    return false;
  }
  const candidate = bytes as Uint8Array;
...
async function computeEnvelopeTag(bytes: Uint8Array): Promise<Uint8Array> {
...
async function parseTextMasterSecretEnvelope(bytes: Uint8Array): Promise<TextEnvelopeInfo | null> {
...
export function describeTextMasterSecret(text: string): TextEnvelopeDescription {
  ensureText(text);
...
export async function encodeTextMasterSecret(text: string): Promise<Uint8Array> {
  ensureText(text);
...
export async function decodeTextMasterSecret(bytes: Uint8Array): Promise<string | null> {
  return (await parseTextMasterSecretEnvelope(bytes))?.text ?? null;
}

export async function isTextMasterSecretEnvelope(bytes: Uint8Array): Promise<boolean> {
